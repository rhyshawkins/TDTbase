...
START_TEST (test_hnk_triangle_create)
{
  hnk_t *triangle;

  triangle = hnk_triangle_create(4, 
				 100,
				 2);
  ck_assert_ptr_ne(triangle, NULL);

  hnk_destroy(triangle);
}
END_TEST

START_TEST (test_hnk_triangle_h3)
{
  hnk_t *triangle;
  ...
  for (i = 0; i < sizeof(kmaxs)/sizeof(int); i ++) {
    ...
    ck_assert(hnk_get_maxk_at_h(triangle, i) == kmaxs[i]);

  }

  for (i = 0; i < sizeof(hnks)/sizeof(int); i ++) {

    ck_assert(hnk_get_hnk(triangle, 2, i, a) == 0);
  ...
  for (i = 0; i < sizeof(ratios)/sizeof(double); i ++) {
    ...
    ck_assert(hnk_get_kplus1_ratio(triangle, 
				   2, 
				   i, 
				   &r) == 0);
...
Suite *
hnk_triangle_suite (void)
{
  Suite *s = suite_create ("HNK Triangle Int");

  /* Core test case */
  TCase *tc_core = tcase_create ("Core");
  tcase_add_test (tc_core, test_hnk_triangle_create);
  tcase_add_test (tc_core, test_hnk_triangle_h3);
