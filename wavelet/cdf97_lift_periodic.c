				    ...
				    double *work)
{
  return generic_lift_forward1d(s, width, stride, work,
				cdf97_lift_periodic_forward1d_cdf97_step);
}
					 ...
					 double *work)
{
  ...
  work[width - 1] += a1 * (work[width - 2] + work[0]);
  
  for (i = 2; i < width; i += 2) {
    work[i] += a2 * (work[i - 1] + work[i + 1]);
  }
  work[0] += a2 * (work[1] + work[width - 1]);
  
  for (i = 1; i < (width - 1); i += 2) {
    work[i] += a3 * (work[i - 1] + work[i + 1]);
  }
  work[width - 1] += a3 * (work[width - 2] + work[0]);
  
  for (i = 2; i < width; i += 2) {
    work[i] += a4 * (work[i - 1] + work[i + 1]);
  }
  work[0] += a4 * (work[1] + work[width - 1]);
				    ...
				    double *work)
{
  return generic_lift_inverse1d(s, width, stride, work,
				cdf97_lift_periodic_inverse1d_cdf97_step);
}
					 ...
					 double *work)
{
  ...
  for (i = 0; i < height; i ++) {
    ...
    cdf97_lift_periodic_forward1d_cdf97_step(s + i*stride,
					     width,
					     1,
					     work);
  ...
  for (i = 0; i < width; i ++) {
    ...
    cdf97_lift_periodic_forward1d_cdf97_step(s + i,
					     height,
					     stride,
					     work);
  }
				...
				double *work)
{
  ...
  for (i = 0; i < height; i ++) {
    ...
    cdf97_lift_periodic_inverse1d_cdf97_step(s + i*stride,
					     width,
					     1,
					     work);
  ...
  for (i = 0; i < width; i ++) {
    ...
    cdf97_lift_periodic_inverse1d_cdf97_step(s + i,
					     height,
					     stride,
					     work);
  }
					 ...
					 double *work)
{
  ...
  for (i = 0; i < height; i ++) {
    for (j = 0; j < depth; j ++) {
      ...
      cdf97_lift_periodic_forward1d_cdf97_step(s + o,
					       width,
					       1,
					       work);
					   ...
					   double *work)
{
   ...
   */
  for (i = 0; i < height; i ++) {
    cdf97_lift_periodic_forward1d_cdf97_step(s + i*rowstride,
				    width,
				    stride,
				    work);
				    ...
				    int subtile)
{
  ...
  if (!subtile) {
     ...
     */
    for (i = 0; i < wlevels; i ++) {
      if (cdf97_lift_periodic_inverse1d_cdf97_step(s, w, 1, work) < 0) {
	return -1;
      }
    ...
    for (i = 0; i < hlevels; i ++) {
      if (cdf97_lift_periodic_inverse1d_cdf97_step(s, h, rowstride, work) < 0) {
	return -1;
      }
    ...
    for (i = 0; i < dlevels; i ++) {
      if (cdf97_lift_periodic_inverse1d_cdf97_step(s, d, slicestride, work) < 0) {
	return -1;
      }
    ...
    for (i = 0; i < whlevels; i ++) {
      if (cdf97_lift_periodic_inverse3d_cdf97_2dstep(s, w, h, 1, rowstride, work) < 0) {
	return -1;
      }
    ...
    for (i = 0; i < wdlevels; i ++) {
      if (cdf97_lift_periodic_inverse3d_cdf97_2dstep(s, w, d, 1, slicestride, work) < 0) {
	return -1;
      }
    ...
    for (i = 0; i < hdlevels; i ++) {
      if (cdf97_lift_periodic_inverse3d_cdf97_2dstep(s, h, d, rowstride, slicestride, work) < 0) {
	return -1;
      }
  ...
  for (i = 0; i <= levels; i ++) {
    ...
    if (cdf97_lift_periodic_inverse3d_cdf97_step(s, w, h, d, rowstride, slicestride, work) < 0) {
      return -1;
    }
					 ...
					 double *work)
{
  ...
  for (i = 0; i < width; i ++) {
    for (j = 0; j < height; j ++) {
      ...
      cdf97_lift_periodic_inverse1d_cdf97_step(s + o,
					       depth,
					       slicestride,
					       work);
					   ...
					   double *work)
{
   ...
   */
  for (i = 0; i < height; i ++) {
    cdf97_lift_periodic_inverse1d_cdf97_step(s + i*rowstride,
					     width,
					     stride,
					     work);
