...
START_TEST (test_haar_matrix_col_step_versus_lift)
{
   ...
   */
  ck_assert(haar_matrix_forward2d_create_col_step(J_MAX, J_MAX, &c) >= 0);
   ...
   */
  for (i = 0; i < width; i ++) {
    ck_assert(haar_lift_forward1d_haar_step(data + i,
					      width,
					      width,
					      workspace) >= 0);
  }

  /* printf("Haar Col Forward: constant\n"); */
  /* for (i = 0; i < size; i ++) { */
  ...
  /* printf("Haar Col Forward: sinusoid\n"); */
  /* for (i = 0; i < size; i ++) { */
  /*   printf("%3d %10.6f %10.6f\n", i, gsl_vector_get(n, i), data[i]); */
  /* } */
...
START_TEST (test_haar_matrix_inverse_col_step_versus_lift)
{
  static const int J_MAX = 3;
  static const double INITIAL_VALUE = 3.1415;
   ...
   * Create an individual row steps
   */
  ck_assert(haar_matrix_inverse2d_create_col_step(J_MAX, J_MAX, &ci) >= 0);
   ...
   */
  for (i = 0; i < width; i ++) {
    ck_assert(haar_lift_inverse1d_haar_step(data + i,
					    width,
					    width,
					    workspace) >= 0);
  }
...
START_TEST (test_haar_matrix_row_step_versus_lift)
{
  static const int J_MAX = 3;
  static const double INITIAL_VALUE = 3.1415;
   ...
   * Create an individual row steps
   */
  ck_assert(haar_matrix_forward2d_create_row_step(J_MAX, J_MAX, &r) >= 0);
   ...
   */
  for (i = 0; i < width; i ++) {
    ck_assert(haar_lift_forward1d_haar_step(data + i*width,
					    width,
					    1,
					    workspace) >= 0);
  }

  /* printf("Haar Row Forward: constant\n"); */
  /* for (k = 0; k < 4; k ++) { */
  /*   for (j = 0; j < width/2; j ++) { */
  /*     for (i = 0; i < width/2; i ++) { */
...
START_TEST (test_haar_matrix_inverse_row_step_versus_lift)
{
  static const int J_MAX = 3;
  static const double INITIAL_VALUE = 3.1415;
   ...
   * Create an individual row steps
   */
  ck_assert(haar_matrix_inverse2d_create_row_step(J_MAX, J_MAX, &ri) >= 0);
   ...
   */
  for (i = 0; i < width; i ++) {
    ck_assert(haar_lift_inverse1d_haar_step(data + i*width,
					      width,
					      1,
					      workspace) >= 0);
  }
...
START_TEST (test_haar_matrix_step_constant)
{
  static const int J_MAX = 3;
  static const double INITIAL_VALUE = 3.1415;
  ...
  ck_assert(haar_matrix_forward2d_create_step(J_MAX, J_MAX, &m) >= 0);
  ck_assert(haar_matrix_inverse2d_create_step(J_MAX, J_MAX, &mi) >= 0);
...
START_TEST(test_haar_matrix_step_sinusoid)
{
  static const int J_MAX = 3;
  ...
  ck_assert(haar_matrix_forward2d_create_step(J_MAX, J_MAX, &W) >= 0);
  ck_assert(haar_matrix_inverse2d_create_step(J_MAX, J_MAX, &Wi) >= 0);
  ...
  ck_assert(haar_lift_forward2d_haar_step(recon, 
					  width,
					  width,
					  width, 
					  work) >= 0);
  ...
  /* For some reason (interlacing? different ordering in subblocks?) the forward lifting
...
START_TEST (test_haar_matrix_constant)
{
  static const int J_MAX = 3;
  static const double INITIAL_VALUE = 3.1415;
  ...
  ck_assert(haar_matrix_forward2d_create(J_MAX, &m) >= 0);
  ck_assert(haar_matrix_inverse2d_create(J_MAX, &mi) >= 0);
...
START_TEST(test_haar_matrix_sinusoid)
{
  static const int J_MAX = 3;
  ...
  ck_assert(haar_matrix_forward2d_create(J_MAX, &W) >= 0);
  ck_assert(haar_matrix_inverse2d_create(J_MAX, &Wi) >= 0);
...
haar_suite (void)
{
  Suite *s = suite_create ("HAAR Matrix");

  /* Core test case */
  TCase *tc_core = tcase_create ("Core");

  tcase_add_test (tc_core, test_haar_matrix_col_step_versus_lift);
  tcase_add_test (tc_core, test_haar_matrix_inverse_col_step_versus_lift);
  
  tcase_add_test (tc_core, test_haar_matrix_row_step_versus_lift);
  tcase_add_test (tc_core, test_haar_matrix_inverse_row_step_versus_lift);
  
  tcase_add_test (tc_core, test_haar_matrix_step_constant);
  tcase_add_test (tc_core, test_haar_matrix_step_sinusoid);
  
  tcase_add_test (tc_core, test_haar_matrix_constant);
  tcase_add_test (tc_core, test_haar_matrix_sinusoid); 
