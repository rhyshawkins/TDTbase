...
START_TEST(test_octahedron_single_step)
{
  ...
  ck_assert(face_wavelet_biohaar_forward_step(o, recon_coeff, DEGREE) >= 0);
  ...
  ck_assert(face_wavelet_biohaar_inverse_step(o, recon_coeff, DEGREE) >= 0);
...
START_TEST(test_octahedron_full)
{
  ...
  ck_assert(face_wavelet_biohaar_forward(o, recon_coeff) >= 0);
  ...
  ck_assert(face_wavelet_biohaar_inverse(o, recon_coeff) >= 0);
  for (i = sstart; i < send; i ++) {
    /* Check for numerical errors */
    ck_assert(isnormal(recon_coeff[i]) || recon_coeff[i] == 0.0);
...
START_TEST(test_octahedron_shell_full)
{
  ...
  ck_assert(face_wavelet_biohaar_shell_forward(o, recon_coeff, total_size, workspace, cdf97_lift_forward1d_cdf97_step) >= 0);
  ...
  ck_assert(face_wavelet_biohaar_shell_inverse(o, recon_coeff, total_size, workspace, cdf97_lift_inverse1d_cdf97_step) >= 0);
  for (j = 0; j < radial_size; j ++) {

    for (i = sstart; i < send; i ++) {
      k = j * o->ntotaltriangles + i;
      /* Check for numerical errors */
      ck_assert(isnormal(recon_coeff[k]) || recon_coeff[k] == 0.0);

      ck_assert(fabs(recon_coeff[k] - input_coeff[k]) < 1.0e-9);
    }
  }
...
START_TEST(test_icosahedron_shell_full)
{
  ...
  ck_assert(face_wavelet_biohaar_shell_forward(o, recon_coeff, total_size, workspace) >= 0);
  ...
  ck_assert(face_wavelet_biohaar_shell_inverse(o, recon_coeff, total_size, workspace) >= 0);
  for (i = 0; i < total_size; i ++) {
    /* Check for numerical errors */
    ck_assert(isnormal(recon_coeff[i]) || recon_coeff[i] == 0.0);
...
Suite *
face_wavelet_biohaar_suite (void)
{
  Suite *s = suite_create ("Face Wavelet");
...
{
  int number_failed;
  Suite *s = face_wavelet_biohaar_suite ();
