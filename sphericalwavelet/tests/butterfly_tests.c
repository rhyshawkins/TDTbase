...
START_TEST(test_octahedron_single_step)
{
  ...
  ck_assert(vertex_wavelet_butterfly_forward_step(o, recon_coeff, DEGREE) >= 0);
  ...
  ck_assert(vertex_wavelet_butterfly_inverse_step(o, recon_coeff, DEGREE) >= 0);
  for (i = 0; i < o->nvertices; i ++) {
    /* Check for numerical errors */
    ck_assert(isnormal(recon_coeff[i]) || recon_coeff[i] == 0.0);
...
START_TEST(test_octahedron_full)
{
  ...
  ck_assert(vertex_wavelet_butterfly_forward(o, recon_coeff) >= 0);
  ...
  ck_assert(vertex_wavelet_butterfly_inverse(o, recon_coeff) >= 0);
  for (i = 0; i < o->nvertices; i ++) {
    /* Check for numerical errors */
    ck_assert(isnormal(recon_coeff[i]) || recon_coeff[i] == 0.0);
...
START_TEST(test_octahedron_shell_full)
{
  ...
  ck_assert(vertex_wavelet_butterfly_shell_forward(o, recon_coeff, total_size, workspace) >= 0);
  ...
  ck_assert(vertex_wavelet_butterfly_shell_inverse(o, recon_coeff, total_size, workspace) >= 0);
  for (i = 0; i < total_size; i ++) {
    /* Check for numerical errors */
    ck_assert(isnormal(recon_coeff[i]) || recon_coeff[i] == 0.0);
