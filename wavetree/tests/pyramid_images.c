...
int main(int argc, char *argv[])
{
  ...
  s = subdivisiontree2d_create(DEGREE, 
			       0.0,     /* Alpha */ 
			       overlap, 
  ...
  if (save_image("pyramid_image_0.txt", width, img) < 0) {
  ...
  if (save_image("pyramid_image_1.txt", width, img) < 0) {
  ...
  if (save_image("pyramid_image_2.txt", width, img) < 0) {
    fprintf(stderr, "error: failed to save depth 1 image\n");
    return -1;
  }
