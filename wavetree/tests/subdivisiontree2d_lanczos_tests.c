...
START_TEST (test_subdivisiontree2d_toplevel_0overlap)
{
  ...
  s = subdivisiontree2d_create(IM_DEGREE, 0.0, 0.0, SUBDIVISION_BASIS_LANCZOS);
  ck_assert(s != NULL);
  ...
  /*
   * Map root node to check top level lanczos
   */
  ...
  for (j = 0; j < (IM_W); j ++) {
    for (i = 0; i < (IM_W); i ++) {
      ...
      ck_assert(img[j*IM_W + i] == basis_lanczos(x, y, 1.0, 0.0));
    }
  }
  ...
  /*
   * Check Top-Left quadrant is a lanczos
   */
  ck_assert(subdivisiontree2d_map_to_array(s, img, IM_SIZE) >= 0);
  ...
  for (j = 0; j < (IM_W/2); j ++) {
    for (i = 0; i < (IM_W/2); i ++) {
      ...
      v = img[j*IM_W + i];
      expected = basis_lanczos(xp, yp, 1.0, 0.0) + 
	basis_lanczos(x, y, 1.0, 0.0);
...
START_TEST (test_subdivisiontree2d_toplevel_halfoverlap)
{
  ...
  s = subdivisiontree2d_create(IM_DEGREE, 0.0, 0.5, SUBDIVISION_BASIS_LANCZOS);
  ck_assert(s != NULL);
  ...
  /*
   * Map root node and check base lanczos
   */
  ck_assert(subdivisiontree2d_map_to_array(s, img, IM_SIZE) >= 0);
  ...
  for (j = 0; j < (IM_W); j ++) {
    for (i = 0; i < (IM_W); i ++) {
      ...
      ck_assert(img[j*IM_W + i] == basis_lanczos(x, y, 1.0, 0.5));
    }
  }
  ...
  for (j = 0; j < (IM_W/2); j ++) {
    for (i = 0; i < (IM_W/2); i ++) {
      ...
      if (fabs(x) <= 2.0 && fabs(y) <= 2.0) {
        ck_assert(img[j*IM_W + i] == (basis_lanczos(x, y, 1.0, 0.5) + 
				      basis_lanczos(xp, yp, 1.0, 0.5)));
      } else {
        ck_assert(img[j*IM_W + i] == basis_lanczos(xp, yp, 1.0, 0.5));
      }
    }
  }
}
...
START_TEST (test_subdivisiontree2d_toplevel_1overlap)
{
  ...
  s = subdivisiontree2d_create(IM_DEGREE, 0.0, 1.0, SUBDIVISION_BASIS_LANCZOS);
  ck_assert(s != NULL);
  ...
  for (j = 0; j < (IM_W); j ++) {
    for (i = 0; i < (IM_W); i ++) {
      ...
      ck_assert(img[j*IM_W + i] == basis_lanczos(x, y, 1.0, 1.0));
    }
  }
  ...
  for (j = 0; j < (IM_W/2); j ++) {
    for (i = 0; i < (IM_W/2); i ++) {
      ...
      if (fabs(x) <= 3.0 && fabs(y) <= 3.0) {
        ck_assert(img[j*IM_W + i] == (basis_lanczos(x, y, 1.0, 1.0) + 
				      basis_lanczos(xp, yp, 1.0, 1.0)));
      } else {
        ck_assert(img[j*IM_W + i] == basis_lanczos(xp, yp, 1.0, 1.0));
      }
    }
  }
}
...
{
  Suite *s = suite_create ("Subdivision Tree 2D: Lanczos Basis");
