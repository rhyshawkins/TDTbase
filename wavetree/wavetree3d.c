...
static int add_node(wavetree3d_t *t, int i, int d, double coeff);
static int remove_node(wavetree3d_t *t, int i, int d);

wavetree3d_t *wavetree3d_create(int degree_width,
				int degree_height,
				int degree_depth,
				double alpha)
{
  ...
  r = malloc(sizeof(wavetree3d_t));
  if (r == NULL) {
    return NULL;
  }

  /* This really only works for square domains, need to think
     about what happens if we have a rectangular case (or other shape) */
  ...
  if (r->degree_depth > r->degree_max) {
    r->degree_max = r->degree_depth;
  }
...
int wavetree3d_valid(wavetree3d_t *t)
{
  ...
  for (d = 1; d <= t->degree_max; d ++) {
	...
	return 0;
      ...
      pi = wavetree3d_parent_index(t, index);
...
{
  wavetree3d_t *t = (wavetree3d_t *)wt;

  return wavetree3d_depthofindex(t, index) == subset;
}
 ...
 */
int 
wavetree3d_choose_birth_depth(const wavetree3d_t *t, double u, int maxdepth, int *depth, double *prob)
{
  int ndepths;
			...
			const multiset_int_double_t *S_vp)
{
  ...
  if (wavetree3d_initialize(t, value) < 0) {
		       ...
		       double *prior_ratio)
{
  ...
  for (d = 0; d < t->degree_max; d ++) {
	...
	return -1;
      ...
      if (wavetree3d_3dindices(t, i, &ii, &ij, &ik) < 0) {
	...
	pi = wavetree3d_parent_index(t, index);
	if (wavetree3d_get_coeff(t, pi, d - 1, &pvalue) < 0) {
...
int wavetree3d_parent_index(wavetree3d_t *t, int c)
{
  ...
  if (wavetree3d_3dindices(t, c, &i, &j, &k) < 0) {
    return -1;
  }
  ...
  return wavetree3d_from_3dindices(t, i, j, k);
}
			    ...
			    int i)
{
  ...
  if (wavetree3d_3dindices(t, i, &ii, &jj, &kk) < 0) {
    return -1;
  }
...
static int r_generate_dyck_word(wavetree3d_t *t, int index, char *buffer, int *len, int maxlength)
{
  int j;

  if (multiset_int_double_is_element(t->S_v, index, wavetree3d_depthofindex(t, index)) == 0) {
...
static int r_generate_dyck_binary(wavetree3d_t *t, int index, uint64_t *binary, int *len)
{
  int j;
...
static int add_node(wavetree3d_t *t, int i, int d, double coeff)
{
  ...
  j = wavetree3d_parent_index(t, i);

  pcc = wavetree3d_child_count(t, j, d - 1);
  if (pcc < 0) {
    return -1;
  }
  ...
  j = wavetree3d_UTL(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_UTR(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_UBL(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_UBR(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_LTL(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_LTR(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_LBL(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }
  j = wavetree3d_LBR(t, i);
  if (j > 0) {
    multiset_int_insert(t->S_b, j, d + 1);
  }    
...
#define WAVETREE3D_CHILD(t, i, di, dj, dk) \
  int ii, ij, ik; \
  \
  if (wavetree3d_3dindices(t, i, &ii, &ij, &ik) < 0) { \
    return -1; \
  ...
  \
  return wavetree3d_from_3dindices(t, ii, ij, ik)
